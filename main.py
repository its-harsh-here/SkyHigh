...
async def enhanced_flight_plan(flight_plan: FlightPlanRequest):
    """Enhanced flight plan analysis with comprehensive weather timeline and NOTAMs"""
    try:
        departure = flight_plan.departure.upper()
        destination = flight_plan.destination.upper()
        waypoints = [wp.strip().upper() for wp in flight_plan.waypoints if wp.strip()]
        ...
        if not departure or not destination:
            raise HTTPException(status_code=400, detail='Departure and destination required')
